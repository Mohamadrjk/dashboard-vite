  ...
  return (
    <div className=" w-[50vw] flex flex-col gap-4  h-[60vh]">
      <div className="  w-full grid grid-cols-2 gap-4">
        {Array.from({ length: 14 }).map((_, index) => <Skeleton.Node key={index} active className="!w-full !h-[40px]" />)}
      </div>
      <div className=" flex justify-end"><Skeleton.Node active className="!w-[200px] !h-[40px]" />
      </div>
    </div>
  )
}
